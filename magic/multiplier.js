...
window.load_modal_content = ( post, post_fields ) => {
  ...
  if ( typeof post.overall_status !== "undefined" ) {
    status_selected = post.overall_status.key
  }
  jQuery.each( post_fields.overall_status.default, function( i, v ) {
    let preselect_s = ''
  ...
  // template
  jQuery('#modal-title').html('<h1>Edit Multiplier</h1><hr>')
  jQuery('#modal-content').append(`
          <div class="grid-x">
            ...
            <div class="cell">
              Note<br>
              <div class="input-group">
                <input type="text" style="border:0; border-bottom: 1px solid darkgrey;font-size:1.5rem;line-height: 2rem;" id="group_description" value="${description}" />
                <div class="input-group-button">
                     <div><span class="loading-field-spinner group_description"></span></div>
                </div>
              </div>
            </div>
            ...
            <div class="cell">
              ...
              <div class="input-group">
                ...
                <div class="input-group-button">
                     <div><span class="loading-field-spinner overall_status"></span></div>
                </div>
              </div>
            </div>
          </div>
    ...
    jQuery('#group_description').on('change', function(e){
      jQuery('.loading-field-spinner.group_description').addClass('active')
      window.post_item('update_description', { post_id: post.ID, new_value: e.target.value } )
        .done(function(result) {
          console.log(result)
          if ( typeof result.errors === 'undefined') {
          ...
          jQuery('.loading-field-spinner.group_description').removeClass('active')
        })
    })
    jQuery('#overall_status').on('change', function(e){
      jQuery('.loading-field-spinner.overall_status').addClass('active')
      window.post_item('update_group_status', { post_id: post.ID, new_value: e.target.value } )
        .done(function(result) {
          ...
          jQuery('.loading-field-spinner.overall_status').removeClass('active')
        })
    })
  ...
  if ( 'map' === jsObject.parts.action ) {
    jsObject.custom_marks.push({lng: lng, lat: lat })
    jQuery(window).on(
